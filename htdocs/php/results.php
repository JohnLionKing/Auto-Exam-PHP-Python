		 ...
		 //    echo "============================================================";
			foreach($_SESSION['FetchedQuestionsSubj'] as $x => $x_value) {
				...
				if (mysqli_num_rows($result) > 0) {
		    		...
		    		while($row = mysqli_fetch_assoc($result)) {
		    			$QuestionName = "Q" . $counter;
						echo "<p>$QuestionName: " . $userAnswer[$QuestionName]."</p>";
						//echo "<p>".$row["Answer"]."</p>";

						$command = 'python question_and_answer.py "' . $row["Answer"] . '" "' . $userAnswer[$QuestionName] .'"';
						$pid = popen( $command,"r");
					    $command_result = fread($pid, 256);
						pclose($pid);
						
						$percent = '';
						if(strlen($command_result) != 0)
						{
							$split_pos = strpos($command_result,";");
							for($i = 0; $i < $split_pos; $i ++)
								$percent .= $command_result[$i];
							$grade = $command_result[strlen($command_result) - 2];
						} else {
							$percent = 0;
						}
						echo "<p>Similarity between user answer and golden answer is: " . $percent . "%</p><p>Thus you answer is ";
