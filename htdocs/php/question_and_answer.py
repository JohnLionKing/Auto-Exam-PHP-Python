...
def main():
    ...
    # print (student_answer_vec)is
    # Get the cosine similarity between 2 vec
    simililarity = cosine_similarity(correct_answer_vec, student_answer_vec)

    # print ("\n\n", int(simililarity*100), "%")
    print (int(simililarity*100))
    print (';')
    # Get the grade from cosine similarity
    if 0 < simililarity <= 0.25:
        grade = 0
    elif 0.25 < simililarity <= 0.5:
        grade = 1
    if 0.5 < simililarity < 0.75:
        grade = 2
    else:
        grade = 3
    # print ("Grade:", grade)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print ("The script require 2 arguements. Invalid Instruction")
        sys.exit(1)
    else:
        correct_answer = sys.argv[1]
        student_answer = sys.argv[2]
